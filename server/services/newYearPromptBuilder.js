...
Местоположение: на улице у Эйфелевой башни, на открытом воздухе, под открытым небом, в Париже на улице, НЕ в помещении, НЕ в доме, НЕ в здании, НЕ в закрытом пространстве. 
Новогодние элементы: Эйфелева башня на улице, новогодние огни на улице, парижская атмосфера на открытом воздухе, праздничное освещение на улице, роскошный декор на улице, небо над головой, открытое пространство. 
...
КРИТИЧЕСКИ ВАЖНО - ЖЕСТКАЯ ФИКСАЦИЯ РАКУРСА: Ракурс головы должен быть СТРОГО ИДЕНТИЧЕН исходному фото. Если в исходнике анфас - СТРОГО анфас без малейших отклонений. Если профиль - СТРОГО профиль без малейших отклонений. Если три четверти - СТРОГО три четверти без малейших отклонений. Если легкий поворот - сохранить ТОЧНО тот же легкий поворот. НЕ поворачивать голову НИ В КАКУЮ СТОРОНУ. НЕ менять угол лица НИ НА ГРАДУС. НЕ менять ракурс головы относительно исходного фото НИ В МАЛЕЙШЕЙ СТЕПЕНИ. Голова должна быть в ТОЧНО ТАКОМ ЖЕ положении относительно камеры, как в исходном фото. Наклон головы должен быть ИДЕНТИЧЕН исходному (если голова прямая - строго прямая; если наклонена - сохранить тот же наклон). 
КРИТИЧЕСКИ ВАЖНО - ЖЕСТКАЯ ФИКСАЦИЯ ВЫРАЖЕНИЯ: Выражение лица должно быть ИДЕНТИЧНО исходному фото. Если в исходнике улыбка - сохранить ТОЧНО такую же улыбку (степень открытости рта, уголки губ, напряжение мышц). Если нейтральное выражение - сохранить СТРОГО нейтральное выражение. Если легкая улыбка - сохранить ТОЧНО такую же легкую улыбку. НЕ изменять выражение лица, НЕ добавлять эмоции, которых нет в исходном фото, НЕ убирать эмоции, которые есть в исходном фото. Мимика должна быть ИДЕНТИЧНА исходному фото во всех деталях (положение бровей, напряжение мышц лица, открытость глаз, положение губ). 
Направление взгляда должно быть ИДЕНТИЧНО исходному (если в исходнике смотрит в камеру - строго в камеру; если смотрит в сторону - в ту же сторону; если смотрит вниз - вниз; если смотрит вверх - вверх). НЕ поворачивать голову для изменения направления взгляда. НЕ менять направление взгляда относительно исходного фото.`;
...
export function buildNewYearPrompts(analysisResult, styleId, locationId) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (isOutdoorLocation) {
      ...
      prompt += ` КРИТИЧЕСКИ ВАЖНО - СТРОГОЕ ПРАВИЛО: На фото должны быть ТОЛЬКО люди и животные из исходного изображения на переднем плане. НЕ добавлять других людей на переднем плане, НЕ добавлять других животных, НЕ добавлять посторонних персонажей на переднем плане. Только те, кто присутствует в исходном фото. КРИТИЧЕСКИ ВАЖНО: Каждый человек из исходного фото должен быть представлен ТОЛЬКО ОДИН РАЗ на каждом изображении. НЕ дублировать людей, НЕ создавать копии людей, НЕ повторять одного и того же человека несколько раз на одном фото. Если в исходном фото 1 человек - на переднем плане должен быть ТОЛЬКО 1 человек. Если в исходном фото 2 человека - на переднем плане должно быть ТОЛЬКО 2 человека (каждый по одному разу). Если в исходном фото 3+ человек - на переднем плане должно быть ТОЛЬКО столько же человек (каждый по одному разу). ИСКЛЮЧЕНИЕ ДЛЯ УЛИЧНЫХ ЛОКАЦИЙ: На заднем плане могут быть другие люди на улице, на открытом воздухе, в массовке, но СТРОГО на открытом воздухе, на улице, под открытым небом. НЕ добавлять людей на заднем плане в помещении, НЕ добавлять людей в закрытых пространствах. Люди на заднем плане должны быть только на улице, на свежем воздухе.`;
    } else {
      // Для помещений: строгий запрет на любых других людей
      prompt += ` КРИТИЧЕСКИ ВАЖНО - СТРОГОЕ ПРАВИЛО: На фото должны быть ТОЛЬКО люди и животные из исходного изображения. НЕ добавлять других людей, НЕ добавлять других животных, НЕ добавлять посторонних персонажей. Только те, кто присутствует в исходном фото. КРИТИЧЕСКИ ВАЖНО: Каждый человек из исходного фото должен быть представлен ТОЛЬКО ОДИН РАЗ на каждом изображении. НЕ дублировать людей, НЕ создавать копии людей, НЕ повторять одного и того же человека несколько раз на одном фото. Если в исходном фото 1 человек - на сгенерированном фото должен быть ТОЛЬКО 1 человек. Если в исходном фото 2 человека - на сгенерированном фото должно быть ТОЛЬКО 2 человека (каждый по одному разу). Если в исходном фото 3+ человек - на сгенерированном фото должно быть ТОЛЬКО столько же человек (каждый по одному разу). НЕ добавлять людей на фон, НЕ добавлять людей вдалеке, НЕ добавлять людей в толпе, НЕ добавлять людей в качестве декораций.`;
    }
