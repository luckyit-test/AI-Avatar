...
export function buildNewYearPrompts(analysisResult, styleId, locationId) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    // Добавляем СТРОГУЮ инструкцию об ориентации в начало и конец промпта для гарантии
    const orientationText = variations.orientation === 'vertical' 
      ? 'КРИТИЧЕСКИ ВАЖНО: Фото должно быть СТРОГО ВЕРТИКАЛЬНЫМ (portrait orientation, вертикальная ориентация, высота больше ширины). НЕ горизонтальное фото.' 
      : 'КРИТИЧЕСКИ ВАЖНО: Фото должно быть СТРОГО ГОРИЗОНТАЛЬНЫМ (landscape orientation, горизонтальная ориентация, ширина больше высоты). НЕ вертикальное фото.';
    
    // Добавляем в начало промпта
    prompt = `${orientationText} ${prompt}`;
    
    // Добавляем ориентацию и технические характеристики в конец промпта
    prompt += ` КРИТИЧЕСКИ ВАЖНО: Ориентация фото: ${variations.orientation === 'vertical' ? 'СТРОГО ВЕРТИКАЛЬНАЯ (portrait, высота больше ширины)' : 'СТРОГО ГОРИЗОНТАЛЬНАЯ (landscape, ширина больше высоты)'}. Технические характеристики: ${variations.cameraSpecs.specs}.`;
