...
async function performImageAnalysis(imageData, type, jobId = null) {
  ...
  if (type === 'evaluate') {
    // Промпт для evaluate-image (новогодние фотосессии)
    const evaluationPrompt = {
      text: `Проанализируй это изображение для новогодней фотосессии. Определи всех людей и животных на фото.

ВАЖНО: Для новогодних фотосессий разрешены:
- Любое количество людей (1, 2, 3, 4+)
- Дети, взрослые и пожилые люди
- Домашние животные (собаки, кошки и т.д.)
- Различные позы и ракурсы

Верни ТОЛЬКО валидный JSON (без дополнительного текста):
{
  "isValid": boolean,  // true если это фото реальных людей/животных и нет запрещенного контента
  "errorType": "none" | "prohibited_content" | "not_photograph" | "license_violation",
  "errorMessage": "строка на русском" (только если isValid: false),
  "peopleCount": число,  // количество людей на фото (1, 2, 3, 4 или больше)
  "animalsCount": число,  // количество домашних животных (0, 1, 2, 3, 4+)
  "totalSubjects": число,  // общее количество субъектов (люди + животные)
  "people": [
    {
      "age": "child" | "adult" | "elderly",  // возраст: ребенок (до ~12), взрослый (13-60), пожилой (60+)
      "gender": "male" | "female" | "unknown",  // пол
      "ageConfidence": число от 0 до 1,  // уверенность в определении возраста
      "genderConfidence": число от 0 до 1  // уверенность в определении пола
    }
  ],
  "animals": [
    {
      "type": "dog" | "cat" | "other",  // тип животного
      "typeConfidence": число от 0 до 1  // уверенность в определении типа
    }
  ],
  "photoQuality": "excellent" | "good" | "fair" | "poor",  // качество фото
  "qualityScore": число от 0 до 1,  // числовая оценка качества (1 = отличное, 0 = плохое)
  "qualityIssues": ["размытие" | "плохое освещение" | "низкое разрешение" | "шум" | "нет"],  // проблемы качества
  "details": {
    "isPhotographOfRealPerson": boolean,  // является ли это фотографией реальных людей (не рисунок/3D)
    "hasProhibitedContent": boolean,  // есть ли запрещенный контент
    "hasMultiplePeople": boolean,  // есть ли несколько людей
    "hasAnimals": boolean,  // есть ли животные
    "isFaceVisible": boolean,  // видно ли лица (не обязательно для валидности)
    "isBlurry": boolean,  // размыто ли фото
    "isDark": boolean  // темное ли фото
  }
}

ПРАВИЛА:
- isValid: true если это фото реальных людей/животных и НЕТ запрещенного контента
- isValid: false только если: не фотография (рисунок/3D/анимация) ИЛИ есть запрещенный контент
- Для новогодних фотосессий разрешены любые комбинации людей и животных
- Возраст определяй строго: ребенок (до ~12 лет), взрослый (13-60), пожилой (60+)
- Если людей больше 4, укажи peopleCount: 4 и totalSubjects: 4+ (или точное число если видно)
- Если животных больше 4, укажи animalsCount: 4+ (или точное число если видно)
- Качество фото оценивай по размытию, освещению, разрешению, шуму`
    };
...
// Robokassa result callback is handled by server/routes/payment.js
// All payment-related endpoints are registered via app.use(paymentRoutes) below
