...
export function initializeTelegramBot(token) {
  ...
  try {
    ...
    bot.start((ctx) => {
      ...
      if (!hasUsedFree) {
        message += '🎁 У тебя есть возможность получить 1 бесплатный портрет!\n\n';
      }
      
      message += '📸 Отправь мне свое фото, и я предложу варианты генерации.\n\n';
      message += 'Используй команду /help для получения справки.';
