...
echo !API_KEY! | plink.exe -ssh root@43.245.226.24 -pw Yd2Vc_Wejus0DlNB "read API_KEY && cd /opt/hnyear && sed -i \"s|GEMINI_API_KEY=.*|GEMINI_API_KEY=\$API_KEY|\" .env && sed -i \"s|GEMINI_API_KEY_ANALYSIS=.*|GEMINI_API_KEY_ANALYSIS=\$API_KEY|\" .env && echo 'API ключи обновлены в .env файле' && echo '' && echo 'Текущие значения:' && grep 'GEMINI_API_KEY' .env | sed 's/=.*/=***/' && echo '' && echo 'Перезапуск backend контейнера...' && docker compose restart backend && echo 'Ожидание запуска backend...' && sleep 5 && echo '' && echo 'Проверка статуса backend...' && docker compose ps backend"
