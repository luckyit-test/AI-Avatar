  ...
  return (
    ...
    >
      {rows.map((row) => {
        ...
        // Начальная позиция: портреты должны начинаться так, чтобы сразу заполнять весь экран
        // Начинаем с позиции, где портреты уже видны по всей ширине контейнера
        const startX = row.direction === 'left' 
          ? containerWidth + rowPadding // Начинаем справа от контейнера, чтобы портреты сразу заполняли весь экран
