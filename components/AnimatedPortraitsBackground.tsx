  ...
  return (
    ...
    >
      {rows.map((row) => {
        ...
        // Начальная позиция: портреты должны сразу заполнять весь экран
        // Чтобы портреты сразу заполняли весь экран при движении влево,
        // они должны начинаться так, чтобы левый край портретов был на левом краю контейнера или левее
        // Это значит: startX должен быть таким, чтобы портреты были видны по всей ширине
        const startX = row.direction === 'left' 
          ...
          : rowPadding; // Начинаем слева, чтобы портреты сразу заполняли весь экран
